...
void MainWindow::on_btnLoad_clicked()
{
    ...
    {
        bool valid = m_image.load(fileName); // -- load image
        if (valid)  //-- check image valid for thr QPixmap
        {
            m_pUI->viewPort->setPrimaryImage(m_image);   //-- image set on the label of the disply window which shows on screen
        }
    }
